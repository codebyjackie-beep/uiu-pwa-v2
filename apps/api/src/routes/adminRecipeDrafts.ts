...
import type { ApiResponse, Recipe, RecipeCost, RecipeDraft } from "@uiu/shared";
...
adminRecipeDraftsRouter.get("/", async (c) => {
  ...
  try {
    const docs = await withDb(c.env, (db) => db.collection("recipe_drafts").find({ status: "pending" }).sort({ createdAt: -1 }).toArray());
